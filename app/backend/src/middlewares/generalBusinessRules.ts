const teamNumbers = (matches: any, teamId: number) => {
  ...
  matches.forEach((match: any) => {
    if (teamId === match.homeTeam) {
      match.homeTeamGoals > 0 ? goalsFavor += match.homeTeamGoals : goalsFavor;
      match.awayTeamGoals > 0 ? goalsOwn += match.awayTeamGoals : goalsOwn;
      match.homeTeamGoals > match.awayTeamGoals ? totalVictories += 1 : totalVictories;
      match.homeTeamGoals === match.awayTeamGoals ? totalDraws += 1 : totalDraws;
      match.homeTeamGoals < match.awayTeamGoals ? totalLosses += 1 : totalLosses;
    };
    if (teamId === match.awayTeam) {
...
export const generalTeamsDashboard = async (finishedMatches: any, allTeams: any) => {
  const teams = allTeams.map((team: any) => {
    const teamName = team.teamName;
    const matches = finishedMatches.filter((match: any) => match.homeTeam === team.id || match.awayTeam === team.id);
    const results = teamNumbers(matches, team.id);
    return {
      name: teamName,
